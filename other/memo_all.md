...
### 登場人物の役割
- Railsアプリケーション
  - ウェイター: お客様（ユーザー）からの注文（リクエスト）を受け付けます。
- ジョブ (Job)
  - 注文伝票: 「ユーザーに歓迎メールを送る」などの具体的な作業指示が書かれています。
- Redis
  - 厨房のオーダーレール: ウェイターが伝票を貼り付ける場所です。
- Sidekiq
  - シェフ: オーダーレールを常に監視し、伝票を取って調理（仕事）をします。


### 処理の流れ
#### ステップ1：Railsアプリケーションがジョブを投入（Enqueue）
- Railsアプリケーション内で、ユーザーのアクションなどをきっかけに「時間のかかる処理」をバックグラウンドで実行するように指示します。
- 出来事: ユーザーが新規登録を完了します。
- コード例(Controller内など)
  ```ruby
  # 「このユーザーに歓迎メールを送って」という注文を依頼
  SendWelcomeEmailJob.perform_later(user.id)
  ```
- 例え: ウェイター（Rails）がお客様から注文（ユーザー登録）を受け、「歓迎メールを送る」という作業内容を注文伝票（ジョブ）に書き出します。


### ステップ2：SidekiqクライアントがジョブをRedisに保存
- `perform_later`が呼ばれると、Railsアプリ内で動いている「Sidekiqクライアント」がその指示をキャッチします。
- 出来事: Sidekiqクライアントは、ジョブの情報（クラス名 `SendWelcomeEmailJob`、引数 `user.id`）を「JSON形式」のデータに変換します。
- コード例(内部的な動き)
  ```json
  // Redisにはこのような形式で保存される
  { "class": "SendWelcomeEmailJob", "args": [123], "queue": "default", ... }
  ```
- 例え: ウェイター（Sidekiqクライアント）が、書き出した注文伝票を厨房のオーダーレール（Redis）にペタッと貼り付けます。これでウェイターの仕事は一旦完了し、すぐ次のお客様の対応に向かえます（ユーザーは待たずに次の画面へ進める）。


### ステップ3：Sidekiqサーバー（Worker）がRedisからジョブを取得
- Railsアプリとは別のプロセスとして、Sidekiqのサーバー（Worker）が常に動いています。
- 出来事: SidekiqのWorkerは、Redisのキュー（`default`キューなど）を常に監視しています。新しいジョブが追加されると、それをキューから取り出します。
- コマンド (Workerの起動)**:
  ```bash
  # このコマンドで起動しているのがSidekiqのサーバープロセス
  bundle exec sidekiq
  ```
- 例え: 厨房のシェフ（Sidekiq Worker）がオーダーレール（Redis）をずっと見ていて、新しい伝票が貼られたのを見つけて手に取ります。

### ステップ4：Workerがジョブを実行（Perform）
- Workerは、取得したジョブのJSONデータを元に、実際の処理を実行します。
- 出来事:
  1.  JSONデータをRubyのオブジェクトに戻します。
  2.  `SendWelcomeEmailJob`クラスの`perform(123)`メソッドを呼び出します。
  3.  `perform`メソッド内に書かれた、メール送信などの時間のかかる処理が実行されます。
- コード例(Jobクラス)
  ```ruby
  ...
  ```
- 例え: シェフ（Sidekiq Worker）が注文伝票（ジョブ）の内容を読み、「歓迎メールを送る」という調理（`perform`メソッド）を開始し、完了させます。
...
1. データセンタープロキシ (Datacenter Proxies)
- IPアドレスの出所: データセンターやクラウドホスティング企業が所有・運営するサーバーから発行されるIPアドレスです。これらは特定の企業に属するIPアドレスとして認識されやすいです。
- メリット:
  - 高速・高帯域幅: データセンターの強力なネットワークインフラを利用するため、通信速度が速く、安定していることが多いです。
  - 安価で大量に入手可能: 他の種類のプロキシに比べてコストが低く、大量のIPアドレスを比較的簡単に入手できます。
  - 管理の容易さ: 通常、IPアドレスは静的（固定）で提供されるため、管理がしやすいです。
- デメリット:
  - ブロックされやすい: IPアドレスの範囲がデータセンターのものとして広く知られているため、多くのウェブサイトでプロキシとして簡単に検知され、アクセスをブロックされたり、CAPTCHAが表示されたりする可能性が非常に高いです。
  - 信頼性が低い: 実際の個人のインターネット利用者のIPアドレスとは異なるため、サイト側からは不審なアクセスと見なされることがあります。
- 主な用途:
  - 速度が重要で、かつターゲットサイトのセキュリティ対策がそれほど厳しくない場合の大量アクセス。
  - CDNのパフォーマンステスト。
  - 一時的なIPアドレス変更。
2. レジデンシャルプロキシ (Residential Proxies)
- IPアドレスの出所: 実際のインターネットサービスプロバイダ (ISP) が、一般家庭のインターネットユーザーに割り当てているIPアドレスです。これらのIPアドレスは、多くの場合、ユーザーの同意を得てP2Pネットワークなどを通じてプロキシとして利用可能になっています（プロバイダによってIPアドレスの調達方法や倫理的側面は異なります）。
- メリット:
  ブロックされにくい（最も大きな利点）: ウェブサイトからは本物の一般ユーザーからのアクセスとして認識されるため、データセンタープロキシに比べて格段にブロックされにくいです。
  - 高い信頼性: ターゲットサイトに対して正規のトラフィックとして振る舞うことができます。
  - 詳細なジオターゲティング: 世界中の国、都市、場合によっては特定のASN（ISP）レベルでIPアドレスを指定できます。
- デメリット:
  - 高価: データセンタープロキシと比較して、料金が大幅に高くなる傾向があります。多くの場合、データ転送量 (GB単位) に基づいて課金されます。
  - 速度・安定性の変動: IPアドレスの出所が一般家庭の回線であるため、データセンタープロキシほど高速で安定しているとは限りません。IPアドレスによって品質にばらつきが出ることがあります。
  - ローテーションが基本: 多くのレジデンシャルプロキシはローテーション型（リクエストごとや一定時間でIPアドレスが自動的に変わる）で提供されます。特定のIPアドレスを長期間固定する「静的レジデンシャルプロキシ」もありますが、さらに高価になる傾向があります。
- 主な用途:
  - Webスクレイピング（特にセキュリティ対策が厳しく、ブロック回避が重要なサイト）。
  - 広告検証（地域別の広告表示確認）。
  - 市場調査（競合サイトの価格情報収集など）。
  - スニーカーボットや限定商品の購入（正規ユーザーとしてのアクセスが求められるため）。
  - 複数アカウントの管理。
3. ISPプロキシ (ISP Proxies、internet service provider)
- IPアドレスの出所: このプロキシは、データセンター内にサーバーが設置されているものの、使用されるIPアドレスは正規のISPから購入・割り当てられたものです。レジデンシャルプロキシとデータセンタープロキシの中間的な特性を持つとされています。
- メリット:
  ブロックされにくさと速度の両立: ISPのIPアドレスであるため、データセンタープロキシよりも格段にブロックされにくいです。同時に、データセンターのインフラを利用するため、レジデンシャルプロキシ（特にローテーション型）よりも高速で安定した通信が期待できます。
  静的IPアドレス: 通常、静的（固定）IPアドレスとして提供されるため、長期間同じIPアドレスを利用したい場合に適しています。
- デメリット:
  - コスト: データセンタープロキシよりは高価で、レジデンシャルプロキシ（ローテーション型）と同等か、場合によってはそれ以上の価格帯になることがあります。IPアドレス単位の月額課金が一般的です。
  - IPプールの規模: レジデンシャルプロキシの巨大なIPプールと比較すると、利用可能なIPアドレスの総数やロケーションの多様性は限定的である場合があります。
  - 匿名性: ローテーション型のレジデンシャルプロキシほど頻繁にIPが変わるわけではないため、匿名性の高さでは劣る場合があります。
- 主な用途:
