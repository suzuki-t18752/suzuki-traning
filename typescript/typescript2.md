...
- 関数式
  ...
  ```
  - 関数式は、別の関数の引数に直接渡すこともできる
  ```
  ボタンがクリックされたときの処理を指定するときの関数式
  ...
  - 型注釈は関数宣言と同じ方法
  - 関数式の関数名
    - function式に関数名を書いた場合、その関数名は関数の処理内部からのみ参照可能
    - 再帰関数を書くときに活用される

## アロー関数 (arrow function)
- functionの定義をより短く記述できる記法
```
(引数) => {
  // 処理内容
};
...
- 省略形
  ...
  ```
  - 戻り値がオブジェクトリテラルの場合はオブジェクトリテラルをカッコ()で囲む必要がある
  ```
  (n) => { foo: n + 1 }; // 誤り
  (n) => ({ foo: n + 1 }); // 正しい
  ...
  ```
- 型注釈
  - 引数のカッコを省略した場合は、引数と戻り値のどちらも型注釈を書けない
```
